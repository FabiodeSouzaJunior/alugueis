...
/**
 * Wraps an API route handler with authentication only.
 * The handler runs with the default service-role clients, so it must enforce
 * organization scoping explicitly using request.auth.organizationId.
 */
export function withAuthContext(handler) {
  return async (request, context) => {
    const auth = await authenticateRequest(request);
    if (auth.error) {
      return NextResponse.json({ error: auth.error }, { status: auth.status });
