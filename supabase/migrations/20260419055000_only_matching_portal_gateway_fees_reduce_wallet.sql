...
begin
  ...
  select
    coalesce(sum(case when entry_type = 'payment_credit' and direction = 'credit' and (source in ('tenant_portal', 'abacatepay_webhook', 'abacatepay_backfill') or coalesce(metadata->>'paymentOrigin', '') = 'tenant_portal') then amount_cents else 0 end), 0),
    coalesce(sum(case when entry_type = 'payment_credit' and direction = 'credit' and (source in ('tenant_portal', 'abacatepay_webhook', 'abacatepay_backfill') or coalesce(metadata->>'paymentOrigin', '') = 'tenant_portal') and public.effective_wallet_entry_available_at(entry_type, available_at, created_at, metadata) > v_now then amount_cents else 0 end), 0),
    coalesce(sum(case when entry_type = 'payment_credit' and direction = 'credit' and (source in ('tenant_portal', 'abacatepay_webhook', 'abacatepay_backfill') or coalesce(metadata->>'paymentOrigin', '') = 'tenant_portal') and public.effective_wallet_entry_available_at(entry_type, available_at, created_at, metadata) <= v_now then amount_cents else 0 end), 0),
    ...
    coalesce(sum(
      case
        when entry_type = 'gateway_fee_debit'
         and (
           source in ('tenant_portal', 'abacatepay_webhook', 'abacatepay_backfill') or
           coalesce(metadata->>'paymentOrigin', '') = 'tenant_portal'
         )
         and (
           checkout_id is null or
           exists (
             select 1
             from public.payment_gateway_checkouts pgc
             where pgc.id = public.wallet_ledger.checkout_id
               and round(coalesce(pgc.amount_fee, 0) * 100)::bigint = public.wallet_ledger.amount_cents
           )
         )
        then case when direction = 'debit' then amount_cents else -amount_cents end
        when entry_type in ('platform_fee_debit', 'withdrawal_debit', 'refund_debit', 'chargeback_debit') then
...
declare
  wallet_row record;
begin
  for wallet_row in select owner_id from public.owner_wallets loop
    perform public.refresh_owner_wallet_summary(wallet_row.owner_id);
  end loop;
end;
$$;
